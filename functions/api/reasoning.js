// Cloudflare Pages Function — /api/reasoning
// Branche le Reasoning Engine pur après Knowledge, sans modifier ses règles.

import { verifyConnectorToken } from "./_auth.js";
import { isValidAnalysisId, loadAnalysisById } from "./analysis/_shared.js";
import { runReasoningForAnalysis } from "../lib/auditComposition.js";
...
function getDb(context) {
  ...
  if (!db) {
    console.error("reasoning: binding ORDERS_DB indisponible.");
    return null;
  }
  return db;
}
...
export async function onRequestPost(context) {
  ...
  const { output } = runReasoningForAnalysis(analysis);
